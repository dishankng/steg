function [ stegoImage, count, messageSize,psnr ] = LSBEncode( cover, messageImage )
...
for x=1:messageSize(1)
    for y=1:messageSize(2)
        for colInd=1:3
            ...
            for it=3:-1:1
                stegoImage(i,j,1)=bitset(stegoImage(i,j,1),it,bitget(colour,it+5));
            end
            for it=3:-1:1
                stegoImage(i,j,2)=bitset(stegoImage(i,j,2),it,bitget(colour,it+2));
            end
            for it=2:-1:1
                stegoImage(i,j,3)=bitset(stegoImage(i,j,3),it,bitget(colour,it));
            end
